...
The `home-routes` handler in the `weather-8.routes.home` namespace
...
The `home-page` function will in turn call the `weather-8.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/weather_8/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `weather-8.handler` namespace:
...
Request middleware functions are located under the `weather-8.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `weather-8.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `weather-8.core` in some other namespace.
